...
def evaluate_metrics(model, dataloader, text_field):
    ...
    with tqdm(desc='Epoch %d - evaluation' % e, unit='it', total=len(dataloader)) as pbar:
        for it, ((images, img_ids, pixels), caps_gt, clip_img, p1) in enumerate(iter(dataloader)):
...
def train_xe(model, dataloader, optim, text_field):
    ...
    with tqdm(desc='Epoch %d - train' % e, unit='it', total=len(dataloader)) as pbar:
        for it, (detections, img_ids, captions, pixels, p1) in enumerate(dataloader):
            ...
            out1 = model(detections, p1, pixels)
            optim.zero_grad()
            p1_gt = p1[:, 1:].contiguous()
            out1 = out1[:, :-1].contiguous()
            loss = loss_fn(out1.view(-1, len(text_field.vocab)), p1_gt.view(-1))
...
def train_scst(model, dataloader, optim, cider, text_field):
    ...
    with tqdm(desc='Epoch %d - train' % e, unit='it', total=len(dataloader)) as pbar:
        for it, ((detections, img_ids, pixels), caps_gt, clip_img, p1) in enumerate(dataloader):
...
def generate_caption(model, dataloader, text_field):
    model.eval()
    with tqdm(desc='Epoch %d - evaluation' % e, unit='it', total=len(dataloader)) as pbar:
        import itertools
        for it, ((images, img_ids, pixels), caps_gt, clip_img, p1) in enumerate(iter(dataloader)):
            ...
            with torch.no_grad():
                out, _ , _= model.beam_search(clip_img, images, pixels, 20, text_field.vocab.stoi['<eos>'], 5, out_size=1)
            caps_gen = text_field.decode(out, join_words=False)
            with h5py.File("semi_model_output.hdf5", 'a') as w:
                index = 0
                for img_id in img_ids:
                    sentence = ""
                    for word in caps_gen[index]:
                        sentence = sentence + word + " "
                    index = index + 1
                    w[str(img_id)] = sentence
...
if __name__ == '__main__':
    ...
    pseudo_cap1_path = args.features_path + '/coco_filter.hdf5'
    pseudo_cap1 = PseudoCaption(detections_path=pseudo_cap1_path)
    p1 = pseudo_cap1
    
    clip_img_path = args.features_path + '/clip_img_feat.hdf5'
    ...
    dict_dataset = dataset.image_dictionary({'image': image_field, 'text': RawField(), 'pixel': pixel_field, 'clip': clip_img, 'p1': p1})
    ...
    dict_dataloader = DataLoader(dict_dataset, batch_size=args.batch_size // 5)
